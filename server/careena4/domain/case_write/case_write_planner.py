from careena4.models.domain import MedicalCase, Observation, Provenance, Subject
...
class CaseWritePlanner:
    def build(
        ...
        *,
        ...
        for index, claim in enumerate(claims.observations):
            ...
            target = self._find_match(medical_case=medical_case, claim=claim)
            if target is None:
    ...
    @staticmethod
    def _find_match(*, medical_case: MedicalCase, claim: ObservationClaim) -> Observation | None:
        identity = (
            claim.type,
