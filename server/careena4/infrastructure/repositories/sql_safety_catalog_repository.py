...
from database.catalog.models import AssessmentCriterion, ConsultationReason, ConsultationReasonAssessmentCriterionLink
...
class SqlSafetyCatalogRepository:
    ...
    def find_matches_for_evidence_terms(self, evidence_terms: list[str]) -> list[SafetyCatalogMatch]:
        normalized_evidence_terms = [self._normalize(term) for term in evidence_terms if term.strip()]
        ...
        with self.session_factory() as session:
            ...
            for link in links:
                ...
                matched = self._find_lay_term_match(normalized_evidence_terms, lay_terms)
                if matched is None:
                    continue
                ...
                matches.append(
                    SafetyCatalogMatch(
                        ...
                        suggested_question_text=self._first_question(criterion.suggested_question_texts_json),
    ...
    def _is_runtime_usable(self, reason: ConsultationReason, criterion: AssessmentCriterion) -> bool:
    ...
    def _find_lay_term_match(self, normalized_evidence_terms: list[str], lay_terms: list[str]) -> tuple[str, str] | None:
        normalized_lay_terms = [self._normalize(term) for term in lay_terms if term.strip()]
        for evidence_term in normalized_evidence_terms:
            for lay_term in normalized_lay_terms:
                if evidence_term == lay_term or evidence_term in lay_term or lay_term in evidence_term:
                    return evidence_term, lay_term
        return None

    def _first_question(self, value: str) -> str | None:
        for question in self._language_list(value):
