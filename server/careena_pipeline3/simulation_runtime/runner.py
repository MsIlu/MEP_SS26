...
from careena_pipeline3.server_log import log_simulation_json, log_simulation_text
from careena_pipeline3.simulation_runtime.adapters.base import ConversationSystemAdapter
    ...
    def run(self, request: SimulationRequest) -> SimulationResult:
        ...
        log_simulation_json(
            "SIMULATION REQUEST",
            {
                ...
                "participant_prompt": request.participant_prompt,
                "opening_message": request.opening_message,
                "max_turns": request.max_turns,
                "participant_llm_mode": request.participant_llm_mode,
                "participant_model": request.participant_model,
                "participant_temperature": request.participant_temperature,
                "default_participant_llm_mode": self.default_participant_llm_mode,
            },
        )
    ...
    def _log_result(self, result: SimulationResult) -> None:
        ...
        log_simulation_json(
            "SIMULATION RESULT",
            {
                ...
                "final_system_response_mode": (
                    result.final_summary.get("response_mode")
                    if isinstance(result.final_summary, dict)
                    else None
                ),
            },
        )
