# Testfaelle Frontend Aufgaben

Diese Datei ordnet die Frontend-Tests des Projekts nach Themen, Testdateien und einzelnen Testfaellen. Die IDs werden in den Testdateien als Referenz verwendet.
...
- `T01.1.2`: Zweimal taegliche Einnahme wird in zwei sortierte Dosiszeilen aufgeteilt.
- `T01.1.3`: Woechentliche und monatliche Plaene werden am Starttag verankert.
- `T01.1.4`: Werktags-Plaene erscheinen nur Montag bis Freitag.
- `T01.1.5`: Taegliche Medikamente bleiben ab dem Erstellungsdatum sichtbar.
...
- `T01.2.1`: JSON-Roundtrip erhaelt Einnahmeplan, zweite Dosis und Einnahmestatus.
- `T01.2.2`: Legacy-Daten ohne neue Felder werden mit sicheren Defaults geladen.
- `T01.2.3`: Woechentliche Medikamente erscheinen nur am verankerten Wochentag.
...
- `T01.4.2`: Checkbox fuer zukuenftige Tage ist deaktiviert.
...
- `T01.5.2`: Untere Aktionen rufen die uebergebenen Callbacks auf.
...
- `T02.1.3`: Titel wird auf ein Wort normalisiert und faellt bei Leerwert auf `Verlauf` zurueck.
...
- `T02.2.1`: Profilhistorie wird vom API-Endpunkt geladen und neueste Eintraege stehen vorne.
...
- `T02.3.2`: Aktive Profil-ID aus der Auth-Session wird an die Chat-API uebergeben.
- `T02.3.3`: Profilwechsel setzt Chat-Session und Symptom-Draft zurueck.
- `T02.3.4`: Normale Empfehlungen werden fuer angemeldete Profile gespeichert und Folgefragen blockiert.
...
- `T03.1.1`: Ohne Session-ID werden keine Symptome geladen und kein API-Call ausgefuehrt.
- `T03.1.2`: Symptome werden fuer eine Session aktualisiert und wieder geladen.
- `T03.1.3`: Draft-Daten koennen geloescht werden.
...
- `T03.1.6`: Fehler beim Loeschen blockieren den Chat nicht.

### T03.2 `app1/test/features/chat/data/chat_api_input_drafts_test.dart`

- `T03.2.1`: Symptome werden ueber `/input-drafts/{sessionId}` geladen.
- `T03.2.2`: Symptome werden per `PATCH` an den Draft-Endpunkt gesendet.
- `T03.2.3`: Drafts werden per `DELETE` am Session-Endpunkt geloescht.
...
- `T03.3.2`: Ohne Symptome wird die Hinzufuegen-Aktion angezeigt.
- `T03.3.3`: Mehr als drei Symptome werden in einer zusammengefassten Gruppe dargestellt.
- `T03.3.4`: Lange Symptom-Labels werden visuell gekuerzt.
...
- `T04.1.3`: Zweiter Registrierungsfall prueft erneut Token-Speicherung und Profilmapping.
...
- `T04.3.2`: Fester Auth-Header haelt den Titel zwischen gleich grossen Aktionsbuttons.
...
- `T04.5.1`: Doppelte E-Mail bei Registrierung wird nutzerfreundlich erklaert.
- `T04.5.2`: Nicht erreichbarer Server bei Registrierung wird nutzerfreundlich erklaert.

## T05 Core Network Und Shared Widgets

### T05.1 `app1/test/core/network/api_client_test.dart`

- `T05.1.1`: Backend-Detail fuer doppelte E-Mail wird in die erwartete Fehlermeldung gemappt.
...
- `T06.1.1`: Nachrichten werden hinzugefuegt, ohne die Ursprungsliste zu mutieren.
...
- `T06.2.3`: Ungueltige Antwort-Payloads nutzen einen lesbaren Fallback.

### T06.3 `app1/test/features/chat/services/chat_service_test.dart`

- `T06.3.1`: `addMessage` haengt an, ohne die bestehende Liste zu mutieren.
...
- `T06.4.3`: `clearSession` gibt die aktuelle Session-ID zurueck und entfernt sie.

### T06.5 `app1/test/features/chat/utils/smart_replies_test.dart`

- `T06.5.1`: Schmerzbezogene Eingaben erzeugen passende Rueckfragen.
- `T06.5.2`: Neutrale Eingaben fallen auf allgemeine Rueckfragen zurueck.

### T06.6 `app1/test/features/chatscreen/widgets/leave_chat_dialog_test.dart`

- `T06.6.1`: Dialog zeigt standardmaessig HomeScreen-Copy.
...
- `T07.1.1`: `HomeFeature`-Konstruktor mappt alle uebergebenen Attribute korrekt.
...
- `T07.2.2`: Einfache Ansicht entfernt Ablenkungen und vergroessert Navigation.
- `T07.2.3`: HomeScreen nutzt die helle Careena-Farbpalette.
- `T07.2.4`: Feature-Karten bleiben im Dark Mode erhoeht.
- `T07.2.5`: Nachrichten-Navigation oeffnet gespeicherte Chat-History.
...
- `T08.1.3`: Profil-Loeschen sendet DELETE mit Authorization-Header.

## T09 Symptom Diary

### T09.1 `app1/test/features/symptom_diary/presentation/controllers/symptom_diary_controller_test.dart`

- `T09.1.1`: Eintraege werden fuer den gewaehlten Tag gespeichert und Durchschnittsintensitaet berechnet.
...
- `T10.1.6`: Managed-Profile-Frontend-Draft wird geoeffnet.
- `T10.1.7`: Personal- und Health-Data-Settings-Seiten werden geoeffnet.
- `T10.1.8`: Suche filtert Settings und laesst Logout sichtbar.
...
- `T11.1.3`: ChatScreen oeffnet mit controller-gestuetzter UI.
...
- `T11.1.7`: ChatWarningDialog schliesst nach Akzeptieren.

## Quality-Prinzipien

- Separation of Concerns: API-Endpunkte, Services, Modelle, Repository, Controller und Widgets werden getrennt getestet.
- KISS: Tests pruefen beobachtbares Verhalten ueber oeffentliche Schnittstellen.
